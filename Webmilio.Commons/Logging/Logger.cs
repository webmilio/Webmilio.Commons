...
namespace Webmilio.Commons.Logging
{
    public class Logger
    {
        ...
        private static readonly ConcurrentDictionary<string, Logger> _loggers = new(new LoggerNameEqualityComparer());
        ...
        public virtual void Log(LogLevel level, string message)
        {
            ...
            message = $"{DateTime.Now.ToLongTimeString()} [{LoggerName}/{level}] : {message}";
            ...
            if (ToConsole)
            {
                ...
                LogCreated?.Invoke(level, message);
            }
        }
        ...
        public static Logger Get<T>(bool toSystemDiagnostics = true, bool toConsole = true) => Get(typeof(T).Name, toSystemDiagnostics, toConsole);
